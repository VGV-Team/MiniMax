...
void miniMaxAI(char board[8][8], struct Figure figures[32], int depth, bool AI = true)
{
	...
	if (myid == 0)
	{
		...
		for (int i = 0; i < size-1; i++)
		{
			...
			//printf("%d %d %d - ", returnValue, bestValue, bestMoveIndex);
			//fflush(stdout);
			
			MPI_Recv(&returnValue, 1, MPI_INT, i + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			
			//printf("%d %d %d\n", returnValue, bestValue, bestMoveIndex);
			//fflush(stdout);
		...
		/*
		for(int qweqweqwe=0; qweqweqwe<1000000; qweqweqwe++)
		{
			//printf("%lf", clock());
		}
		*/
...
int main(int argc, char *argv[])
{
	...
	if (myid == 0)
	{
		...
		*/
		...
		printf("TOTAL %lf MOVES %d\n", totalTime, numofMoves);
	...
	else
	{
		...
		while(1)
		{
			...
			fflush(stdout);
			MPI_Send(&retInt, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
		}
